...
## Recent Changes
- 026-generate-uml-diagram: Right-click "Generate UML Diagram" (`ontograph.generateUmlDiagram`, same `view/item/context` availability as "Open Graph") opens a webview showing a UML-style diagram rooted at the selected class — composition (part-of, filled diamond) and generalization (subtype, hollow triangle) connectors derived purely from the ontology's own axioms, no AI/LLM involvement. `src/uml/partOfGraph.ts` extracts via two distinct traversals (one-hop ancestor pre-pass + multi-hop downward BFS with a node cap), resolving SNOMED's clinical/continuant split (`resolveAnchor`) so the diagram operates entirely in "Entire X" concept space; `src/uml/layout.ts` computes a deterministic tidy-tree layout (`LayoutDirection: 'TB' | 'LR'`, default `'LR'`); `src/uml/diagramGeometry.ts`/`htmlRenderer.ts` render a server-computed HTML/SVG fragment the webview injects directly (no client-side layout logic); `src/uml/drawioRenderer.ts`/`drawioCli.ts` back draw.io/SVG/PNG export from the same data. `ontograph.umlDiagram.compositionProperties` (default: SNOMED's four part-of properties) and `ontograph.umlDiagram.defaultDepth` settings; in-webview controls for depth, layout direction, node exclusion (`src/uml/nodeExclusion.ts`, subtree/splice modes), and hiding SNOMED Left/Right lateralized variants by default. See `specs/026-generate-uml-diagram/tasks.md`'s twelve post-delivery addenda for the full incremental history (bug fixes to edge routing/box-crossing/marker collisions, branch coloring, sibling reordering, the lateralized-classes toggle, etc.) and `specs/026-generate-uml-diagram/plan.md`/`spec.md` for the original design.
