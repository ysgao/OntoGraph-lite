...
- 032-uml-inferred-subtypes: the UML diagram view gains a "Stated"/"Inferred" switch (`webview-src/uml/UmlDiagramApp.ts`, a genuine `<select>`, not a checkbox) that shows two COMPLETELY SEPARATE diagrams, never mixed — Stated defaults to selected and is `extractUmlDiagram()`'s exact pre-feature asserted-axiom behavior (fully reverted, zero inferred-hierarchy awareness); Inferred is a new sibling function, `extractInferredUmlDiagram()` (`src/uml/partOfGraph.ts`), whose entity scope comes ENTIRELY from `model.inferredSubClasses` (gated on `model.isClassified`, never triggers classification) — generalization-only (no composition/"part of" at all), rooted at the clicked entity directly (no "All or part of" anchor-hop to "Entire X", unlike Stated), and default-excluding both lateralized (Left/Right) variants AND classes labeled "Entire X" (revealable via the same existing toggle used for lateralized classes elsewhere). A relationship confirmed by reasoning but with no supporting asserted axiom renders dashed (`isInferred` flag on `DiagramEdge`/`RenderedSegment`, `stroke-dasharray="3 3"` / `dashPattern=3 3`, distinct from the far-edge `6 4` pattern) in the webview, standalone SVG, and draw.io exports; one that's ALSO directly asserted renders solid. New `stripStructureLabel()` (Inferred-view-only) simplifies "Kidney structure"/"Structure of kidney"-style labels down to "Kidney". Mode state (`ViewMode = 'stated' | 'inferred'`, `src/views/UmlDiagramMessages.ts`) is per-focus-session, mirroring the lateralized toggle's reset-on-refocus/close lifecycle. → `specs/032-uml-inferred-subtypes/`
- 031-layered-graph-layout: replaces the UML diagram's old flat/reactive layout with a proper Sugiyama-style layered graph algorithm across `src/uml/layout.ts`, `layerCoordinates.ts`, `layerOrdering.ts`, `dummyNodes.ts`, `busLanes.ts`, and `diagramGeometry.ts`. New `assignTidyTreeCoordinates` centers each parent exactly at the midpoint of its children's span (a Reingold–Tilford-style tree walk reduced to a spanning tree; sibling parents are spaced unevenly, by subtree width, not an even pitch). New `busLanes.ts`'s `assignBusLanes` colors bus lanes by interval overlap, shared between `layout.ts` (which sizes each transition's flow-gap to the resulting lane count) and `diagramGeometry.ts` (which places each bus at its lane's height) — two buses only need distinct heights when their horizontal spans genuinely overlap (disjoint same-kind buses now share a lane, compacting inter-level height) or when they're of different composition/generalization kinds (always kept on separate heights). Multi-layer ("far") edges route through reserved dummy-node columns and their own dashed, uniformly-spaced lanes below the ordinary buses (`routeFarEdgesThroughLanes`). Each edge is colored by the node it connects to (target for a single destination, source when fanning out to several). A node entered by 2+ edges assigns each edge a distinct connection point, ordered by its source's cross-position, so incoming lines no longer swap sides and cross. → `specs/031-layered-graph-layout/`
- 030-sync-labels-in-axioms: renaming an entity's label no longer leaves *other* entities' cached Entity Editor display showing the stale label. New `findEntitiesReferencingIri` (`src/model/OntologyIndex.ts`) reverse-scans all axiom-bearing fields for a given IRI (mirrors `updateIriReferencesInModel`'s scan style); new `invalidateEntries` (`src/views/EntityEditHistory.ts`) removes matching entries from `entityHistoryMap`; both are wired together via `invalidateHistoryAfterLabelChange` (`EntityEditorPanel.ts`), called from the `save` handler whenever `msg.labels` actually changes — so `sendLoadEntity`'s existing map-miss fallback re-renders fresh with the current label on next view. Also blocks any rename that would duplicate another entity's existing label outright (`checkLabelRenameConflict`, reusing `OntologyIndex.exactMatchByLabel`), surfaced via a new `LabelRenameResultMessage` (mirrors `IriRenameResultMessage`) and a `label-conflict-banner` in the webview. Applies to every format the panel already supports for label edits (`.ofn`/`.omn`/`.ttl`), not just `.ofn`. → `specs/030-sync-labels-in-axioms/`
- 029-delete-entity-subtypes: right-click "Delete Entity" (or the `$(remove)` "-" toolbar button in each tree view's title bar, acting on the view's current selection — mirrors the existing `$(add)` "+" add-entity toolbar buttons) on any class/object/data/annotation-property/individual tree item. Entities with direct subtypes prompt a mode choice — "delete entity only" (default, reparents direct subclasses/sub-properties to the deleted entity's own supers) vs "delete entity and all subtypes" (cascade, full transitive closure). New `getDirectSubtypes`/`getTransitiveSubtypes` in `src/model/OntologyIndex.ts` (also now used by `entity-info`'s `directSubClasses`, replacing its old inline loop). New `src/sync/EntityDeletionSync.ts` (reparenting-rule mutation, protected-root check, Declaration/header-line removal, blank-line collapse) plus `src/commands/deleteEntity.ts`, reusing `computeUpdatedText` (`EntityEditorPanel.ts`) for axiom/annotation-line removal rather than touching `AxiomSync.ts`/`AnnotationSync.ts` directly. Functional syntax (`.ofn`) only in this release, matching `019-create-entity`'s same scoping precedent. → `specs/029-delete-entity-subtypes/`
...
<!-- SPECKIT START -->
Plan: specs/032-uml-inferred-subtypes/plan.md
<!-- SPECKIT END -->
